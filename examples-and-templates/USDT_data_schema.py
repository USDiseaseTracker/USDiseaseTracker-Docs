...
sub_state_jurisdictions: dict[str, list[str]] = {
    ...
    "MI": [
        "Region 1", "Region 2 North", "Region 2 South", "Region 3", "Region 5", "Region 6", "Region 7", "Region 8"
    ],
    "MN": [
...
sub_state_jurisdictions = {k: v + ["unknown", "unspecified"] for k, v in sub_state_jurisdictions.items()}
        ...
        if geo_unit == "state":
            ...
            if v not in known_jurisdictions:
                raise ValueError(
                    ...
                    f"\nexpected one of: {known_jurisdictions[:1] + ['...'] + known_jurisdictions[-3:]}"
                )
